...
function one_solve(fname, fout; log=true, algo_bb =false, algo_eps=false, heur=false, preproc=0)
    ...
    # set_attribute(model, MOA.Tolerance(), 1.0e-5)
...
function run(fname, method)
    ...
    folder = "./resFractional/Gurobi"
    if !isdir(folder)
        mkdir(folder)
    end

    if method == "epsilon"
        folder = "./resFractional/Gurobi/epsilon"
        if !isdir(folder)
            mkdir(folder)
        end
    ...
    if method == "bb"
        folder = "./resFractional/Gurobi/bb"
        if !isdir(folder)
            mkdir(folder)
        end
    ...
    if method == "bb_heur"
        folder = "./resFractional/Gurobi/bb_heur"
        if !isdir(folder)
            mkdir(folder)
        end
    ...
    if method == "bb_heur_preproc1"
        folder = "./resFractional/Gurobi/bb_heur_preproc1"
        if !isdir(folder)
            mkdir(folder)
        end
    ...
    if method == "bb_preproc1"
        folder = "./resFractional/Gurobi/bb_preproc1"
        if !isdir(folder)
            mkdir(folder)
        end
    ...
    if method == "bb_heur_preproc2"
        folder = "./resFractional/Gurobi/bb_heur_preproc2"
        if !isdir(folder)
            mkdir(folder)
        end
    ...
    if method == "bb_preproc2"
        folder = "./resFractional/Gurobi/bb_preproc2"
        if !isdir(folder)
            mkdir(folder)
        end
